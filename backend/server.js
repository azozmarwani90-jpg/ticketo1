...
app.delete('/api/bookings/:id', (req, res) => {
  try {
    ...
    const bookingIndex = db.bookings.findIndex((b) => String(b.id) === String(bookingId));
...
app.post('/api/events', (req, res) => {
  try {
    const { title, category, city, venue, date, time, price, image, description, lat, lng } = req.body || {};
    
    if (!title || !category || !city || !venue || !date || !time) {
      res.setHeader('Content-Type', 'application/json');
      return res.status(400).json({ ok: false, error: 'Missing required fields' });
    }
    
    const db = readDB();
    const newId = Math.max(0, ...db.events.map((e) => Number(e.id) || 0)) + 1;
...
app.put('/api/events/:id', (req, res) => {
  try {
    ...
    const eventIndex = db.events.findIndex((e) => String(e.id) === String(event.id));
    ...
    db.events[eventIndex] = {
      ...
      lat: lat !== undefined ? (lat != null ? Number(lat) : null) : event.lat,
      lng: lng !== undefined ? (lng != null ? Number(lng) : null) : event.lng
    };
    
    writeDB(db);
    
    res.setHeader('Content-Type', 'application/json');
    res.json({ ok: true, event: db.events[eventIndex] });
  } catch (error) {
